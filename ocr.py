...
Historique : le projet est passé un temps par PaddleOCR (moteur par deep
learning), puis est revenu à Tesseract à la demande explicite du porteur du
projet — Tesseract reste le moteur utilisé en production. Voir
ocr_benchmark/RAPPORT_PaddleOCR_vs_Tesseract.md pour la comparaison faite
entre les deux, et GUIDE_TESSERACT_INSTALLATION.md pour l'installation.

extract_text(), normalize_text() et extract_text_normalized() gardent
exactement la même signature quel que soit le moteur derrière : le reste du
pipeline (classifier.py, zones.py, extractor.py, validator.py) n'a besoin
d'aucun changement.
"""
